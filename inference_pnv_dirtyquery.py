...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate PointNetVLAD recall on KITTI with dirty query / clean db.")
    parser.add_argument(
        "--query-file",
        type=str,
...
def main() -> None:
    ...
    dirty_object_insertion = not args.no_dirty_object_insertion
    if not args.no_dirty_object_insertion:
        dirty_object_insertion = not bool(dirty_saved_args.get("no_object_insertion", False))
    ...
    db_features, db_key_order, _ = extract_embeddings(
        dataset=db_dataset,
        model=model,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        device=device,
        desc="Extract clean db embeddings",
    )
